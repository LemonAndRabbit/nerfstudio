...
class TensoRFWithMLP3DModel(Model):
    ...
    def get_training_callbacks(
        ...
        def update_alpha_mask(self, step: int):
            print(f"========> updating alpha mask at step {step} ...")
            ...
            if step > self.config.update_alpha_mask_iters[0]:
                xyz_in_occupancy = (xyzs - self.occupancy_grid._roi_aabb[:3]) / (
                    self.occupancy_grid._roi_aabb[3:] - self.occupancy_grid._roi_aabb[:3])
                xyz_in_occupancy = xyz_in_occupancy * 2 - 1
                alpha_mask = torch.zeros(
                    (cur_reso[0], cur_reso[1], cur_reso[2]), dtype=torch.bool, device=xyzs.device)
                for i in range(cur_reso[0].item()):
                    sampled_alpha = F.grid_sample(self.occupancy_grid._binary.transpose(0, 2)[None, None, ...].float(
                    ), xyz_in_occupancy[i].view(1, -1, 1, 1, 3), align_corners=True).view(-1)
                    alpha_mask[i] = sampled_alpha.view(
                        cur_reso[1], cur_reso[2]) > 0
            else:
            ...
            ks = 3
            alpha = F.max_pool3d(alpha, kernel_size=ks, padding=ks //
                                 2, stride=1).view((cur_reso[0], cur_reso[1], cur_reso[2]))

            if step == 4000:
                for base in [1e-4, 1e-3, 1e-2, 1e-1]:
                    for factor in range(1, 10):
                        threshold = base * factor
                        tmp_alpha = alpha.clone()
                        tmp_alpha[tmp_alpha < threshold] = 0
                        tmp_alpha[tmp_alpha >= threshold] = 1
                        tmp_alpha = tmp_alpha*alpha_mask
                        valid_xyzs = xyzs[tmp_alpha > 0.5]
                        try:
                            tmp_aabb = torch.cat(
                                [valid_xyzs.amin(0), valid_xyzs.amax(0)]).view((2, 3))
                            print(
                                f"threshold: {threshold}, Alpha mask ratio: {tmp_alpha.mean()}, new occupied aabb: {tmp_aabb}")
                        except IndexError as err:
                            print(err, valid_xyzs)
                            print(
                                f"threshold: {threshold}, Alpha mask ratio: {tmp_alpha.mean()}, new occupied aabb: None")
            ...
            self.occupancy_grid._binary = alpha.to(
                device=self.occupancy_grid._binary.device, dtype=torch.bool)
        ...
        def shrink_tensorf_grids(self, step: int):
            ...
            print(
                f"  Occupancy grid safety check:{self.occupancy_grid._roi_aabb}")
        ...
        def filter_training_rays(self, training_callback_attributes: TrainingCallbackAttributes, step: int):
            ...
            assert training_callback_attributes.pipeline.datamanager.train_image_dataloader.cache_all_images, \
                NotImplementedError(
                    "Current ray filtering only works with cache_all_images=True")
            ...
            N = cached_data['image'].shape[0] * \
                cached_data['image'].shape[1] * cached_data['image'].shape[2]
            ...
            for idx_chunk in idx_chunks:
                ...
                if step == 0:
                    ...
                    mask_inbbox = torch.zeros(
                        chunk_all_rays.origins.shape[0], dtype=torch.bool)
                    mask_inbbox[chunk_ray_indices.unique()] = True

                mask_filtered.append(mask_inbbox.cpu())

            mask_filtered = torch.cat(mask_filtered).view(
                cached_data['image'].shape[:-1] + (1,))
    ...
    def update_to_step(self, step: int, state_dict) -> None:
        ...
        if '_model.occupancy_grid._binary' in state_dict:
            self.occupancy_grid._binary = state_dict['_model.occupancy_grid._binary'].clone(
            ).to(self.device)
    ...
    def populate_modules(self):
        ...
        if self.config.tensorf_encoding == "vm":
            ...
            raise ValueError(
                f"Encoding {self.config.tensorf_encoding} not supported")

        feature_encoding = NeRFEncoding(
            in_dim=self.appearance_dim, num_frequencies=2, min_freq_exp=0, max_freq_exp=2)
        ...
        if self.yzf_mode:
            self.sampler = UniformSampler(
                num_samples=self.config.num_uniform_samples, single_jitter=True)
        elif self.yzf_mode2:
            flat_aabb = Parameter(
            ...
            self.sampler_pdf = PDFSampler(num_samples=self.config.num_samples,
                                          single_jitter=self.config.enable_single_jitter, include_original=False)

        # renderers
        self.renderer_rgb = RGBRenderer(background_color=colors.WHITE)
        self.renderer_accumulation = AccumulationRenderer()
        self.renderer_depth = DepthRenderer(
            method="expected" if self.yzf_mode2 else "median")
    ...
    def get_outputs_mode1(self, ray_bundle: RayBundle):
        ...
        acc_mask = torch.where(coarse_accumulation <
                               0.0001, False, True).reshape(-1)

        field_outputs_fine = self.field.forward(
            ray_samples_uniform, mask=acc_mask, bg_color=colors.WHITE.to(
                weights.device)
        )
    ...
    def get_outputs_old(self, ray_bundle: RayBundle):
        ...
        # fine field:
        field_outputs_fine = self.field.forward(
            ray_samples_pdf, mask=acc_mask, bg_color=colors.WHITE.to(
                weights.device)
        )
