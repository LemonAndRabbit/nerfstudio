...
from nerfstudio.field_components.encodings import Encoding, Identity, SHEncoding, NeRFEncoding
from nerfstudio.field_components.field_heads import FieldHeadNames, RGBFieldHead, DensityFieldHead
from nerfstudio.field_components.mlp import MLP
from nerfstudio.fields.base_field import Field


class TensoRFMLP3DField(Field):
    """TensoRF Field"""
    ...
    def __init__(
        ...
        self.mlp_head = MLP(
            in_dim=appearance_dim + 3 + self.direction_encoding.get_out_dim() +
            self.feature_encoding.get_out_dim()+256,
        ...
        self.bottleneck = nn.Linear(self.mlp_base.get_out_dim(), 256)
        ...
        assert use_sh is False, "Spherical harmonics are not currently supported with TensoRF MLP3D Field"
        self.use_sh = use_sh
        ...
        if self.use_sh:
            ...
            self.B = nn.Linear(in_features=self.color_encoding.get_out_dim(
            ), out_features=appearance_dim, bias=False)
    ...
    def get_density(self, ray_samples: RaySamples) -> TensorType:
        ...
        if torch.min(torch.tensor(positions.size())).item() > 0:
            encoded_xyz = self.position_encoding(positions)
            base_mlp_out = self.mlp_base(encoded_xyz)
            base_mlp_out = base_mlp_out.to( 
                encoded_xyz.dtype)  # FIX dtype mismatch error
            mlp_density = self.field_output_density(
                base_mlp_out)
        else:
            mlp_density = torch.zeros(
                ray_samples.frustums.shape[:-1]).to(density.device)

        density_enc = torch.sum(density, dim=-1)[..., None]+mlp_density
        density_enc = self.density_activation(
            density_enc + self.density_offset)
    ...
    def get_outputs(self, ray_samples: RaySamples, density_embedding: Optional[TensorType] = None) -> TensorType:
        ...
        if torch.min(torch.tensor(positions.size())).item() > 0:
            encoded_xyz = self.position_encoding(positions)
            base_mlp_out = self.mlp_base(encoded_xyz)
            base_mlp_out = base_mlp_out.to(encoded_xyz.dtype)
            rgb_feat = self.bottleneck(base_mlp_out)
        else:
            rgb_feat = torch.zeros(
                ray_samples.frustums.shape[:-1]).to(rgb_features.device)
        ...
        if self.use_sh:
            ...
            out = self.mlp_head(torch.cat(
                [rgb_features, d, rgb_features_encoded, d_encoded, rgb_feat], dim=-1))  # type: ignore
    ...
    def forward(
        ...
        if mask is not None and bg_color is not None:
            base_density = torch.zeros(ray_samples.shape)[
                :, :, None].to(mask.device)
    ...
    def shrink_grid(self, new_aabb: torch.Tensor) -> torch.Tensor:
        ...
        tl, br = (xyz_min - old_aabb_min) / \
            unit_size, (old_aabb_max-xyz_max)/unit_size
